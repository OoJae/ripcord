...
description: >
  ...
  Clash Display in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  ink-black: "#EDEAE3"
  ink-black-alt: "#EDEAE3"
  fire-orange: "#FF4F00"
  cream: "#0A0B10"
  cream-muted: "#8A8F9C"      # brand --dim
  phosphor: "#7CF5C4"         # SCOPED: the recovered/verified state only, never decoration
  cream-hint: "#5A5F6C"
  border-dark: "#23262F"      # brand --hairline
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Satoshi", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Satoshi", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Satoshi", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "Space Mono", cqw: 0.72, weight: 400, tracking: "0.14em", upper: true }
  # — display / hero ramp (Clash Display, lowercase, negative tracking) —
  h3:      { fontFamily: "Clash Display", cqw: 2.8, weight: 600, lineHeight: 1.2, upper: true }
  quote-text:{ fontFamily: "Clash Display", cqw: 3.8, weight: 600, lineHeight: 1.15, tracking: "-0.02em", upper: true }
  h2:      { fontFamily: "Clash Display", cqw: 4.5, weight: 600, lineHeight: 1.1, tracking: "-0.02em", upper: true }
  stat-value:{ fontFamily: "Clash Display", cqw: 5.5, weight: 600, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Clash Display", cqw: 7.5, weight: 600, lineHeight: 0.9, tracking: "-0.03em", upper: true }
  fadelist-item:{ fontFamily: "Clash Display", cqw: 7.5, weight: 600, lineHeight: 1.0, tracking: "-0.03em", upper: true }
  quote-mark:{ fontFamily: "Clash Display", cqw: 10.0, weight: 600, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Clash Display", cqw: 10.5, weight: 600, lineHeight: 0.9, tracking: "-0.04em", upper: true }
  display: { fontFamily: "Clash Display", cqw: 13.0, weight: 600, lineHeight: 0.88, tracking: "-0.04em", upper: true }
...
- **Fonts** — already set to **Clash Display** (display) / **Satoshi** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** Clash Display `display` at 13cqw puts a single lowercase word
...
**Clash Display** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Clash Display 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Clash Display body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Clash Display `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Clash Display display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Clash Display `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Clash Display lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Clash Display `display`/`h1`
...
row of three top-border-only stat-cards — big Clash Display-900 numeral in `{colors.fire-orange}`, Clash Display
...
**Ground** ink-black. **Composes** fadelist (3 stacked Clash Display-900 words at 1.0/0.5/0.22), fadelist-title.
...
Clash Display `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Clash Display display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Clash Display display; never add a second accent color.
...
- No serif companion; chrome is never Clash Display.
...
- **Type** — Clash Display lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Clash Display + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
...
```html
<style>
@font-face{font-family:"Clash Display";font-weight:600;font-style:normal;font-display:block;src:url("assets/fonts/ClashDisplay-Regular.woff2") format("woff2");}
